...
\begin{minipage}[t]{0.195\linewidth}
	...
	\begin{betterlist}
		\item \nul{State-based execution:} Transition between current and the next states is indicated with -->. E.g. Main.Running --> (Main.Waiting, A.Blocked), \cul{path of execution} is described by a sequence of individual program states, \nul{Trace-based description of program execution}, \cul{Deadlock:} When all threads are blocked, Go runtime system recognizes and aborts, \cul{Starvation:} Thread does not progress, because the other threads always communicate with each other, \cul{Livelock:} Situation in which always at least one thread is not blocked, but no thread progresses, \cul{Preemptive scheduling}, \cul{Cooperative scheduling}, Multithreading, \cul{Concurrency}, \cul{Parallelism}, pre(ch?/!): \nul{wanting to receive / send} on channel ch, post(ch?/!) \cul{having received / sent} on channel ch, post(ch!) < post(ch?), \nul{only sending:} func snd(ch chan <- int) {}, \nul{only receiving:} func rcv(ch <- chan int) {}, \cul{Dynamic analysis}: Execute the program and observe its behavior, Determine if there is any potential bad behavior based on this specific program run, \cul{Static analysis}: Predict all possible program runs without actually executing the program, events are collected in a program trace. \cul{trace}: Linear sequence of events and represents an interleaved execution of the program (of individual threads), an \cul{event} registers interesting program behavior, two events are \cul{conflicting events} if they are read/write events for the same variable that come from different threads and at least one of them is a write event, a \cul{data race} arises if two conflicting events appear right next to each other in the trace. That implies that both events may happen in any order, \nul{All events are post events. x++ is w(x)}, \cul{valid trace reorderings}: Reordering of a trace means that we change the order of its events such that the resulting sequence of events still represents a sensible execution sequence, \cul{Complete}: All valid reorderings that exhibit some race can be predicted. If a method is \nul{incomplete}, we call every race that is not reported a \nul{false negative}, \cul{Sound}: All races reported by the method can be exhibited with some valid reordering of the trace. If a method is \nul{unsound}, it may report a trace as racy even if there is no valid reordering that exhibits a race. Such a report is a \nul{false positive}. \cul{Resource deadlock} If a set of threads are blocked and each thread in the set is waiting to acquire a lock held by another thread in the set, whenever we say deadlock we refer to a resource deadlock, \cul{communication deadlocks}: If threads are blocked due to (missing) channel events such as send and receive, a \cul{promise} is a placeholder for a computation that is explicitly provided by the programmer. A \cul{future} can be viewed as a placeholder for a computation that will eventually become available. A future is a \cul{specific kind of promise} where the computation is provided at initialization, \cul{Shared Memory Concurrency}: Processes interact by reading and writing shared variables, Locking etc. needed to demarcate critical regions, \cul{Message Passing Concurrency}: processes interact by sending and receiving messages on shared communication channels
% Each event is connected to one of the operations we are interested in
		% \begin{betterlist}
...
\begin{minipage}[t]{0.195\linewidth}
	...
	\begin{enumerate}
		...
		\end{betterlist}
		\item \cul{Fork-join condition:} \nul{Fork-1:} For each thread T != T1 (not the main thread), there exists at most one fork(T) in P, and there is no fork(T1) in P., \nul{Fork-2:} For each event e in thread T != T1, there exists fork(T) in thread T′ != T and fork(T) < Pe., \nul{Join:} For each join(T) in thread T' we have T != T' and, for each event e in T, e < Pjoin(T)
	\end{enumerate}
	\inlinebox{Lockset Method cont.}
	\begin{betterlist}
...
func wait(qsem *QSem) {
  ...
  if qsem.curr > 0 {
    ...
    %\end{noindent}
			...
			%     client(2, req)
			% }
			\inlinebox{Non-determ. Choice (\texttt{select})}
			\vspace{-0.25cm}
...
func signal(qsem *QSem) {
  ...
  if qsem.curr < qsem.q {
	...
	\begin{betterlist}
		...
		\item \cul{unsound}: Like HB, the lockset method ignores write-read dependencies (therefore the HB unsoundness also applies)
		\begin{betterlist}
			\item lockset enables reordering of critical sections. By reordering critical sections (to exhibit the data race) may run into a deadlock
			\item fork information is not recorded
		\end{betterlist}
		...
		\begin{betterlist}
			...
			\item To be \nul{efficient}, in an implementation based on the lockset method each thread maintains a list of the most recent reads/writes and their locksets
		\end{betterlist}
	\end{betterlist}
	\inlinebox{Lamport's happens-before (HB, incomplete and unsound) relation}
	\begin{betterlist}
...
\begin{minipage}[t]{0.195\linewidth}
	\raggedright
	\inlinebox{Lamport's happens-before relation cont.}
	\begin{betterlist}
		...
		\begin{enumerate}
			...
			\end{betterlist}
			\item \nul{If fork(T)} €  P, then, for each e € P arrodown T, fork(T) < e. \nul{If join(T)} €  P, then, for each e €  P arrowdown T, e < join(T)
			% Recall thread projection: P arrowdown T is the list of events in thread T contained in trace P. 
		\end{enumerate}
	...
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item for each thread only keep most recent \cul{timestamp}. E.g. in case of \{1\#\{1, 2\}\} we write \{1\#2\}
		\end{betterlist}
	\end{betterlist}
\end{minipage}
\begin{minipage}[t]{0.195\linewidth}
	...
	\begin{betterlist}
		...
		\item \nul{Precision:} \cul{False positive} possible. All events take place in the same thread. Common guard lock z.
		% \begin{betterlist}
		% 	\item Deadlock prediction based on lock graph for Go-style mutexes results in further false positives
		% \end{betterlist}
	\end{betterlist}
	\inlinebox{False positive lock dependency and lock graph}
	\vspace{-0.25cm}

  \begin{minipage}[t]{0.58\textwidth}
	\adjustbox{scale=0.52}{
		% \begin{noindent}
		  \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\Large}, box align=top]
T1    T2       // Same thread 
acq(y)          acq(y)
...
\begin{minipage}[t]{0.195\linewidth}
	...
	\begin{betterlist}
		...
		\item \nul{state variables}: \cul{ls(t)}: The set of locks hold by thread t at a certain time, \cul{Ds}: The set of lock dependencies
...
\begin{minipage}[t]{0.195\linewidth}
	...
	\adjustbox{scale=0.48}{
        ...
        func example1() {
          ...
          p2 := newPromise[int]()
          p2.complete(booking)
          // p1 := future[int](booking)
          // p2 := future[int](booking)
          p := p1.firstSucc(p2)
        ...
        func (p *Promise[T]) onSuccess(
         cb func(T)) {
          p.m <- 1
          if p.empty {
            p.succCallBacks = append(
             p.succCallBacks, cb)
        ...
        func (p *Promise[T]) onFailure(
         cb func()) {
          p.m <- 1
          if p.empty {
            p.failCallBacks = append(
             p.failCallBacks, cb)
          } else if !p.empty && !p.status {
            go cb()
          } else {}
        ...
        func (p1 *Promise[T]) firstSucc(
         p2 *Promise[T]) *Promise[T] {
          p := newPromise[T]()
...
\begin{minipage}[t]{0.195\linewidth}
	...
	\begin{betterlist}
		...
		\item inf. path fragment = state0 state1 state2  \ldots |\raisebox{0.4mm}{=} phi iff \cul{trace(inf. path fragment) |}\raisebox{0.4mm}{=} \cul{phi} iff \cul{trace(path) }€ \cul{Words(phi)} (inf. path fragment doesn't need to start at initial state)
		\item \cul{state |}\raisebox{0.4mm}{=} \cul{phi} iff trace(path) |\raisebox{0.4mm}{=} phi for all path € Paths(state) iff \cul{Traces(state) C}\raisebox{0.3mm}{=} \cul{Words(phi)}
		\item \cul{T |}\raisebox{0.4mm}{=} \cul{phi} iff s\textsubscript{0} |\raisebox{0.4mm}{=} phi for all s\textsubscript{0} € S\textsubscript{0} iff trace(path) |\raisebox{0.4mm}{=} phi for all path € Paths(T) iff \cul{Traces(T) C}\raisebox{0.3mm}{=}\cul{ Words(phi)} iff T |\raisebox{0.4mm}{=} Words(phi)
		...
		% \item TODO: Die Sache mit free variables und free names bei Pi Calculus
		% \item TODO: den einen Pi Calculus Beweis
	\end{betterlist}
	\inlinebox{Pi Calculus}
	\begin{betterlist}
		\item \nul{Syntax:}
		\begin{betterlist}
			\item u, v ::= a, b, c, \ldots (names € N) | x, y, z, \ldots (variables)
			\item pi ::= u<v> (send list of names v along channel u) | u(y) (receive list of names y along channel u) | tau (unobservable action)
			\item P ::= +\textsubscript{i€I} p\textsubscript{i}.P\textsubscript{i} (summation over finite index set I) | P | Q (parallel composition) | (nu a) P (restriction (binds a name)) | !P (replication)
			\begin{betterlist}
				% \item 0 stands for the empty sum (i.e., I = {}) pi.P abbreviates a singleton sum
				% \item u(−) and (nu −) are \cul{binders}
				\item \nul{Recursion by Replication:} A = acode.A(x).acode -> A' = acode.a<x>.acode, P = pcode.A(x).pcode -> P' = pcode.a<x>.pcode, A' | P' -> (nu a)(P' | !a(x).A')
			\end{betterlist}
		\end{betterlist}
		\item \cul{alpha-conversion} (written P =alpha  Q) is the consistent renaming of \nul{bound variables} or \nul{bound names}. It must not change or hide free variables/names
		\item a \cul{substitution} [x := a] applied to a process P (as in P[x := a]) replaces all free occurrences of \nul{variable} x by name a. Substitution is \cul{capture-avoiding}, that is, it alpha-converts bound names as needed.
...
\begin{minipage}[t]{0.195\linewidth}
	...
	\inlinebox{Pi Calculus}
	\begin{betterlist}
		\item \cul{Structural Congruence (Axioms) cont.:}
		\begin{betterlist}
...
%     %\end{noindent}
		...
		%   %\end{noindent}
		% }
		\item \nul{server with help of unrestricted receive:}
		\vspace{-0.25cm}

		\adjustbox{scale=0.5}{
			% \begin{noindent}
    \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\Large}, box align=top]
(nu d1 d2)( // mu s.un!(mu a.lin+{max: lin!integer.lin!integer. 
           //                  lin?integer.a, quit: end}).s
  (nu client server)( // mu a.lin+{max: lin!integer.lin!integer.
                   //          lin?integer.a, quit: end}
    un d2?s.s > {max: s?x.s?y if x > y then s!x.d1!s else
                s!y.d1!s, quit: 0}
    | d1!server.client < max.client!-4.client!2.client?x
     .printInteger!x.client > quit
  )
)
    \end{dnumberedcodebox}
    %\end{noindent}
...
%     %\end{noindent}
		% 		}
		\item \nul{shared channel with unrestricted type:}
		\vspace{-0.25cm}

		\adjustbox{scale=0.5}{
			% \begin{noindent}
		  \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\Large}, box align=top]
		    (nu x1 x2)( // x1 = mu a.un ?bool.a = *un?bool
		      lin x1(y) | x2<true> | x2<true>
		    )
		  \end{dnumberedcodebox}
		  %\end{noindent}
		}
		% \item \nul{linear channel with linear / unrestricted type:}
		% \vspace{-0.25cm}
		%
		% \adjustbox{scale=0.5}{
		% 	% \begin{noindent}
	...
	\inlinebox{False negative HP and False positive lockset}
	\vspace{-0.25cm}

	\begin{minipage}[t]{0.6\textwidth}
	\adjustbox{scale=1}{
		% \begin{noindent}
		  \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\scriptsize}, box align=top]
T1   T2        LS      x = 3      
w(x)             {}      acq(y)     
acq(y)                    rel(y)     
rel(y)                     go func() {
...
rel(y1)
	...
	\inlinebox{False pos. HP, lockset and false neg. lock graph, lock dep.}
	\vspace{-0.25cm}

...
\begin{minipage}[t]{0.19\linewidth}
    ...
    %\end{noindent}
		...
		\item \nul{recursion with unrestricted type:}
		\vspace{-0.25cm}
		...
		\adjustbox{scale=0.5}{
			% \begin{noindent}
    \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\large}, box align=top]
      (nu client server)( // mu s.un+{max: lin?(lin!integer.lin!integer.lin?integer.end).s}
                       //          quit: end}
      (nu d1 d2)( // mu a.un!a.a
      un d2(e)(
      server > {max:
                (nu writer reader)( // lin!integer.lin!integer.lin?integer.end
                server!writer.reader?x.reader?y if x > y then reader!x else reader!y),
                quit: 0} | d1<e>) | d1<d2>)
      | client > max.client?c.c!-4.c!2.c?x.printInteger!x.client > quit
      | client > max.client?c.c!3.c!7.c?x.printInteger!x.client > quit)
    \end{dnumberedcodebox}
    %\end{noindent}
		}
		\item \nul{recursion with linear type:}
		\vspace{-0.25cm}

		\adjustbox{scale=0.5}{
			% \begin{noindent}
    \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\large}, box align=top]
(nu client server) // mu s.lin +{max: lin!integer!integer.?integer.s,
                //           quit: end}
client < abs.client!-4.client!2.client?x. printInteger!x.client < quit
| (nu d1 d2)( // mu a.un!a.a
  un d2(e)(server > {max server?x.server?y.if x > y then server!x else server!y,
                   quit: 0} | d1<e>) | d1<d2>)
    \end{dnumberedcodebox}
    %\end{noindent}
		}
		\item \nul{unrestricted syntax with linear type:}
		\vspace{-0.25cm}

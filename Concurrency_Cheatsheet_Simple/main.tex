...
\begin{minipage}[t]{0.19\linewidth}
    ...
    \item path = state0 state1 state2  \ldots |\raisebox{0.4mm}{=} \cul{phi} iff \cul{trace(path) |}\raisebox{0.4mm}{=} \cul{phi} iff \cul{trace(path) }€ \cul{Words(phi)}
    \item \cul{state |}\raisebox{0.4mm}{=} phi iff trace(path) |\raisebox{0.4mm}{=} phi for all path € Paths(state) iff \cul{Traces(state) C}\raisebox{0.3mm}{=} \cul{Words(phi)}

          ...
          \resizebox{\textwidth}{!}{
            \begin{minipage}[t]{12cm}
              \begin{itemize}
                ...
                \begin{itemize}
                  \item AES needs fitting definitions of $+$ and $\cdot$ on $\{0, 1\}^8$ to turn it into a \alert{finite field}
                    ...
                    \begin{itemize}
                      \item \script{134}{Definitions (ff.)}
                        \begin{itemize}
                          ...
                          \item $\boxed{12 / 10 = 1 \wedge 12 \mod 10 = 2 \enspace(\text{rest})} \Rightarrow 2 \equiv 12 \mod 10 \Leftrightarrow 2 + 1 \cdot 10 = 12 + 0 \cdot 10 \Leftrightarrow \boxed{2 + 1 \cdot 10 = 12}\Leftrightarrow 2 + 2 \cdot 10 = 12 + 1 \cdot 10$ ($12$ is the dividend, because it's larger than $2$)
                        \end{itemize}
                      ...
                      \begin{itemize}
                        ...
                        \item \script{148}{Definition: Residue Class Ring Modulo a Polynomial}: $\overline{F(X)} = F[X] / g(X):=\{\overline{u(X)} \mid u(X) \in F[X]\}$
                          \begin{itemize}
                            ...
                            \item \script{148}{Addition and Multiplication in $F[X]/g(x)$}: $\overline{u(X)}+\overline{v(X)}:=\overline{u(X)+v(X)}$ and $\overline{u(X)} \cdot \overline{v(X)}:=\overline{u(X) \cdot v(X)}$
                              \begin{itemize}

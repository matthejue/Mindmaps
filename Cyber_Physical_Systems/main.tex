						...
						child {
								node {Classification of LT-properties
										\resizebox{\textwidth}{!}{
											\begin{minipage}[t]{12cm}
												\begin{itemize}
													\item \script{703}{Overview and examples}, classification does not mean that we have here the set of all LT properties all linear Time Properties and then we have then we can divide all linear Time Properties such that it is either a safety or liveness that is not the case so it's not as if LT is the union of safety and liveness however what is the case is that whenever I have a property e that I can write this e as a disjunction of E1 and E2 such that E1 is a safety property and E2 is a liveness property and this is very nice that I can do this so it's a form of decomposition I decompose the property into a conjunction of a safety and the liveness property and this is very nice because I have one algorithm for safety properties and I have another algorithm for liveness properties
												\end{itemize}
											\end{minipage}
										}
									}
												...
												\resizebox{\textwidth}{!}{
													\begin{minipage}[t]{12cm}
														\begin{itemize}
															...
															\begin{itemize}
																\item \script{811}{Proof (ff.)}
															\end{itemize}
															\item \underline{examples:}
															\begin{itemize}
																\item \script{830}{Every invariant is a safety property}
																\item \script{837}{$\emptyset$ is a safety property and $(2^{AP})^\omega$ is a safety property}
															\end{itemize}
														\end{itemize}
													\end{minipage}
												}

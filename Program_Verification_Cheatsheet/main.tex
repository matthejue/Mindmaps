...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\end{betterlist}
		\item Program Automaton $\mathcal{A}_P$: Let P be a program with assert statements, and let $G = (Loc, \Delta , \ell_{init}, \ell_{ex}, Loc_{err})$ be a CFG with error locations for $P$. The \alert{program automaton} for $P$ is the NFA $AP = (Loc, \sum , \Delta , \{ \ell_{init}\} , Loc_{err})$ where $\sum$ is the set of all statements $st$ such that $(\ell, st, \ell') \in \Delta $for some locations $\ell, \ell'$
		\begin{betterlist}
			\item $P$ satisfies all assert statements iff every word in $L(AP)$ is infeasible
		\end{betterlist}
	...
	\begin{betterlist}
		...
		\item Lemma Farkas: If $\exists\vec{x} . A \cdot \vec{x} \leq\vec{b}$ is satisfiable, then $\forall\vec{x} . (A \cdot \vec{x} \leq\vec{b} \rightarrow\vec{c}^T\cdot \vec{x} \leq \delta)$ is equivalent to $\exists\vec{\lambda} . ( \vec{\lambda} \geq 0 \land\vec{\lambda}^T\cdot A =\vec{c}^T\land\vec{\lambda}^T\cdot \vec{b} \leq \delta)$
	\end{betterlist}

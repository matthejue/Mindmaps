...
\begin{mindmap}
	\begin{mindmapcontent}
		...
		child {
				...
				child {
						node {Gradient Descent}
						% if gradient very small then probably very close to the minimum
						% take gradient and multiply with step size
						% gradient gives direction and magnitude of how big the slope is, use this to know how much should take step towards the minimum
						% if want to minimize that then have to go in opposite direction of the slope
						% why: if change w one step to the left then have positive gain in J(w), so have increase in J(w), so tangent is positive
						% derivative tells direction in which the function goes up
						% gradient is derivative of function j of w with respect to each of the w's, tells tangent line
						child {

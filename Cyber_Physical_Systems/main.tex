...
\begin{mindmap}
	\begin{mindmapcontent}
		...
		child {
				node (software model checking) {Software Model Checking
						\resizebox{\textwidth}{!}{
		...
		child {
				node (model checking) {Model Checking
						\resizebox{\textwidth}{!}{
				...
				child {
						node (ltl model checking) {LTL model checking
								\resizebox{\textwidth}{!}{
									\begin{minipage}[t]{18cm}
										\begin{itemize}
											\item \script{1647}{Illustration (pr.)} and \script{1664}{Illustration counter example}, \script{1742}{Illustraction more details (prr.)}
											\item \script{1736}{Problem}
											\item \script{1672}{Algorithm (prr.)}, \script{1739}{Algorithm counterexample}
											\item \script{1674}{Example (ff.)}
											\item \script{1752}{Comparison with regular safety model checking}
											\begin{itemize}
												\item \script{1756}{Equivalences comparison}
											\end{itemize}
										\end{itemize}
									\end{minipage}
								}
						...
						child {
								node (model checking ltl with fairness) {Model checking for LTL with fairness
										\resizebox{\textwidth}{!}{
											\begin{minipage}[t]{12cm}
												\begin{itemize}
													\item \script{1690}{Definition (prr.)}
												\end{itemize}
											\end{minipage}
										}
									}
				...
				child {
						...
						child {
								...
								child {
												...
												\resizebox{\textwidth}{!}{
													\begin{minipage}[t]{20cm}
														\begin{itemize}
															...
															\item \script{790}{Definition (ff.)} and \script{1287}{Summary (f.)}, set of all \alert{infinite} words / traces that do not have a bad prefix
															\begin{itemize}
										...
										child {
												node (nfa and nba for safety properties) {NFA and NBA for language of bad prefixes of a safety property
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{16cm}
																\begin{itemize}
								...
								child {
										...
										child [level distance = 8cm] {
														...
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	...
																	\item \script{1693}{Summary (f.)}
																\end{itemize}
															\end{minipage}
														}
													}
												child {
														node (fairness in ltl) {Fairness in LTL
																\resizebox{\textwidth}{!}{
																	\begin{minipage}[t]{12cm}
																		\begin{itemize}
																			\item \script{1344}{LTL fairness assumptions}
																			\item \script{1698}{Conjunctions of LTL formulas (prr.)}
																			\item \script{1702}{Fair satisfaction relation}
																			\begin{itemize}
																				\item \script{1707}{Randomized arbiter for MUTEX (ff.)}
																				\item \script{1717}{Examples}
																			\end{itemize}
																			\item \script{1728}{Equivalence (pr.)}
																			\begin{itemize}
																				\item \script{1726}{Examples: Weak and strong fairness (prr.)}
																				\item \script{1729}{Example: Weak until and normal until}
																			\end{itemize}
																			\item \script{1731}{$\varphi_1 \equiv_{fair} \varphi_2$ implies $\neg\varphi_1 \equiv_{fair} \neg\varphi_2$}
																			\item \script{1733}{(Usecases)}
																		\end{itemize}
																	\end{minipage}
																}
															}
												...
												child {
														node {Fairness assumptions
																\resizebox{\textwidth}{!}{
																	\begin{minipage}[t]{16cm}
																		\begin{itemize}
																			\item \script{1176}{Definition}, Unconditional, strong and weak fairness
																			\begin{itemize}
	...
	\begin{edges}
		...
		\edge{software model checking}{model checking}
	\end{edges}

...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\item \enquote{Synthesize} ranking functions analogously to the synthesis of invariants
		\item \script{563}{Example for just ranking function (ff.)}
		\item in lasso programs the loop sometimes does not have a ranking function but there is a ranking function for the combination of the loop and a given Floyd-Hoare annotation
		\item in lasso programs the loop sometimes does not have a ranking function but there is a ranking function for the combination of the loop and a given Floyd-Hoare annotation. For these programs we synthesize a ranking function together with a loop invariant
		\item \script{571}{Constraints for ranking function and invariant}
		\begin{betterlist}
			\item The constraints for the special case where we are searching for a ranking function of the form $f(\vec{v}) =\vec{r}^T\cdot \vec{v} + r_0$ and an invariant of the form $\vec{s}^T\cdot \vec{v} + s_0 \geq 0$
		\end{betterlist}
		\item Synthesis of ranking functions available in Ultimate: LassoRanker
	\end{betterlist}
	\fbox{Terminator}
	\begin{betterlist}
  ...
  if (*) {
    ...
    %\end{noindent}
		...
		\begin{betterlist}
			...
			\item Termination analysis available in Ultimate: Büchi Automizer
		\end{betterlist}

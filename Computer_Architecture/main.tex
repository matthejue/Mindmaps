            ...
            node {Normalised Numbers
              \resizebox{\textwidth}{!}{
                \begin{minipage}[t]{11cm}
                  \begin{itemize}
                    \item $x = ( -2)^S \times (1 + Fraction) \times 2^{Exponent - Bias}$, $\boxed{\frac{0}{1}\mid 1101\mid 010}$
                    \item $Bias = 1^{e-1} - 1$, 7, $\boxed{0111}$
                    \item \alert{Smallest normalised number} $\pm 0.125 \times 2^{1-7}$, $\boxed{\frac{0}{1}\mid 0001\mid 000}$
                    \item \alert{Largest normalised number} $\approx \pm 0.875 \times 2^{14-7}$, $\boxed{\frac{0}{1}\mid 1110\mid 111}$
                    \item \alert{Normalising:} $-1.011 \cdot 2^2 \cdot \frac{2^{-4}}{2^2} = 1.100 \cdot 2^{-6}$
                  \end{itemize}
                \end{minipage}
              }
            }
          ...
          child {
            node {GRS Bits
              \resizebox{\textwidth}{!}{
                \begin{minipage}[t]{8cm}
                  \begin{itemize}
                    \item \alert{Guard bit:} The first of two extra bits kept on the right during intermediate calculations of floating- point numbers; used to improve rounding accuracy.
                    \item \alert{Round bit:} Method to make the intermediate floating-point result fit the floating-point format; the goal is typically to find the nearest number that can be represented in the format. It is also the name of the second of two extra bits kept on the right during intermediate floating- point calculations, which improves rounding accuracy.
                    \item \alert{Sticky bit:} A bit used in rounding in addition to guard and round that is set whenever there are nonzero bits to the right of the round bit.

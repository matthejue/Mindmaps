...
\begin{mindmap}
	\begin{mindmapcontent}
				...
				child [level distance=100cm] {
										...
										child [level distance=20cm] {
												...
												child {
														...
														child {
																node {Types}
																child [level distance=10cm] {
																		node (output layer) {Output (Layer) Activation fuctions}
																		child {
						...
						child [level distance=100cm] {
								...
								child {
										node {Loss Function}
										child [level distance=10cm] {
												node (continous target regression) {Continous Target, Regression}
												child {
		...
		child {
				...
				child {
						node (l2) {Least Squares (L2)}
						child {
								node {Maximum Likelihood Estimation
									}
							}
            child {
              node {Increases quadratically with error}
            }
					}
				child {
						node {L1}
            child {
              node {Increases only linearly with error}
              child {
                node {In presence of outliers better suited}
              }
            }
					}
			}
		...
		child [level distance=10cm] {
				node {Strategies}
				child {
						node {One-vs-All (OvA) / One-vs-Rest (OvR)}
						child {
								node {Train K classifiers}
								child {
										node {Each classifier tries to separate one class from all the other classes combined}
									}
							}
						child {
								node {Class with the highest score is chosen}
							}
					}
				child {
						node {One-vs-One (OvO)}
						child {
								node {Train a classifier for every pair of classes}
								child {
										node {Each classifier only distinguishes between two classes at a time}
									}
								child {
										node {$\frac{K \times (K - 1)}{2}$}
									}
							}
						child {
								node {Class with the most votes is chosen}
							}
					}
			}
		child {
				node (multiclass classification) {Multi-class Classification}
				child {
						node (log-likelihood loss) {Log-likelihood Loss / Logloss}
            child [level distance=20cm] {
		...
		child {
		node (logistic loss) {Logistic Loss or the Cross-entropy}
		child [level distance=30cm] {
		...
		child [level distance=20cm] {
            ...
            child {
              node {Replacing the single model with an average ensemble \alert{reduces the variance}}
            }
						child {
								...
								% TODO: Question to prof: not aggregated?
								child {
										node {Regression: Averaging}
									}
								child {
                    ...
                    child {
                      node {If errors are uncorrelated then the error is reduced linearly}
                    }
										child {
				...
				child [level distance=135cm] {
						node {Boosting}
						child {
								node {\alert{\textbf{Sequentially}} train a weak model that tries to fix the problems of all previous ones, while keeping them fixed}
								child {
										node {Resulting ensemble is the weighted sum of the sub-models}
									}
							}
						child {
								node {Sequentially aggregates high-bias models to create a \alert{low-bias ensemble}}
								child {
						...
						child {
								...
								child {
										node {Works with any differentiable loss function}
									}
								child [level distance=30cm] {
										node {Gradient Boosted Decision Trees}
										child {
												node {Instantiation of Gradient Boosting}
											}
										child {
												node {Hyperparameters}
												child {
										...
										child {
												node {Algorithm}
												child {
														node {Optimal leaf weight}
														child {
																node {Second-order Taylor approximation + regularization}
																child {
																		node {Take gradient, set = 0 and solve for w}
																	}
															}
													}
												child {
														node {Optimal objective function}
														child {
																node {Used as gain cuz allows to determine does split reduce loss more than keeping the node as is}
															}
														child {
																node {Calculate gain}
																child {
																		node {Determine G and H}
																	}
																child {
																		node {Calculate G and H for all features and insert in gain calculation}
																	}
															}
														child {
																node {Inserting optimal leave weight into objective function}
																child {
																		node {Get best possible contribution of that leaf to the loss reduction}
																	}
															}
													}
											}
										child {
												node {Complexity}
												child {
														node {$O(k\cdot M\cdot N\cdot log(T))$ with k trees}
													}
											}
										child {
												node {Advantages}
								...
								child {
										...
										child {
												node {Special case of Gradient Boosting}
											}
										child {
												...
												child {
														node {Minimizes weighted particular loss}
														child {
																node {Original works with loss function for binary classification}
																child {
																		node {Instance-penalty loss that is approximated by expoential loss}
																	}
															}
													}
											}
		...
		child [level distance=150cm] {
				node {Hyperparameter Optimization}
				child {
						node {Hyperparameters usually cannot be determined analytically}
					}
				child {
						node {Response function $f\colon \Lambda \times \mathcal{D} \to \mathbb{R}$}%$f: \Lambda \times \mathcal{D} \to \mathbb{R}$}
						child {
								node [text width=6cm, align=center] {
										$\lambda^* = \arg\min_{\lambda \in \Lambda} \mathcal{L}\big(\hat{y}_\lambda^{\text{opt}}(D^{\text{train}}), D^{\text{val}}\big)$\\
										$= \arg\min_{\lambda \in \Lambda} f(\lambda, D)$
									}
								child {
										node {Search for configuration yielding a learned model with best validation performance}
									}
							}
						%         child {
						%           node {$\lambda^\star := \arg\min_{\lambda \in \Lambda} 
						% \mathcal{L}\big(\hat{y}_\lambda^{\text{opt}}(D^{\text{train}}), D^{\text{val}}\big) 
						% := \arg\min_{\lambda \in \Lambda} f(\lambda, D)$}
						%         }
					}
				child {
	...
	\begin{edges}
		...
		\edge{log-likelihood loss}{multi-class}
		\edge{binary}{logistic loss}
		\edge{continous target regression}{l2}
		\edge{softmax}{output layer}
		\edge{sigmoid}{output layer}
	\end{edges}
\end{mindmap}
\end{document}

% TODO: Was ist mit SVM und anderen Modellen, kann man mit denen regression, multiclass und binary classification und ranking machen?
% TODO: begriffe wie multilayer perceptron und perceptron? Verbindung NN und SVMs
% TODO: Linear fixen, loss functions
% TODO: boosting redue variane, bagging redue bias oder so
% TODO: Ensemble reduces variance?

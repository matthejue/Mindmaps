...
\begin{minipage}[t]{0.19\linewidth}
	...
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item <>phi = true U phi (eventually)\quad []phi = true U phi (always)
		...
		% 	\item Aj Aj+1 Aj+2 \ldots |\raisebox{0.5mm}{=} phi
		% \end{betterlist}
		\item sigma |\raisebox{0.5mm}{=} []<>phi iff there are \cul{infinitely many} j >= 0 s.t. A j A j+1 A j+2 \ldots |\raisebox{0.5mm}{=} phi
		\item sigma |\raisebox{0.5mm}{=} <>[]phi iff for \cul{almost all} j >= 0 we have Aj Aj+1 Aj+2 \ldots |\raisebox{0.5mm}{=} phi
	\end{betterlist}
	\inlinebox{Tricks}
	\begin{betterlist}

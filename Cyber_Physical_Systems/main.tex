						...
						child {
								node {Classification of LT-properties
										\resizebox{\textwidth}{!}{
											\begin{minipage}[t]{12cm}
												\begin{itemize}
													\item \script{703}{Overview and examples} and \videoeight{3935}{Explanation}, classification does not mean that we have here the set of all LT properties all linear Time Properties and then we have then we can divide all linear Time Properties such that it is either a safety or liveness that is not the case so it's not as if LT is the union of safety and liveness however what is the case is that whenever I have a property e that I can write this e as a conjunction of E1 and E2 such that E1 is a safety property and E2 is a liveness property and this is very nice that I can do this so it's a form of decomposition I decompose the property into a conjunction of a safety and the liveness property and this is very nice because I have one algorithm for safety properties and I have another algorithm for liveness properties, to prove that a transition system satisfies E I prove that it satisfies E1 and I prove that it satisfies E2, to prove safety properties I take the finite prefixes I look at the finite words and for the for the other ones I look at the infinite words, for the finite ones I use automata over finite Words and for the other ones the second kind of life properties one will use Büchi automata
												\end{itemize}
											\end{minipage}
										}
									}
								child {
										node {Safety properties
												\resizebox{\textwidth}{!}{
													\begin{minipage}[t]{18cm}
														\begin{itemize}
															\item \script{790}{Definition (ff.)}
															\begin{itemize}
																\item \videoeight{2065}{Explanation: Counter Example}
																\item \videoeight{2283}{Explanation: Empty Set}
																\item \scriptthree{4}{More direct} and \videoeight{2109}{Explanation}, whenever I have a trace that does not satisfy the property I know after finetely many states or finetely many elements in the sequence I know it means that I no longer need to look at what the remainder of Sigma is because the remainder of Sigma does not matter whatever you choose to extend it you will not be able to get a trace that satisfies E you will not be able to repair it
																\item another way to read this is if a trace violates the property e then it has a bad prefix but what is a bad prefix well it is one that cannot be extended to trace E
															...
															\begin{itemize}
																\item \script{779}{Alternative depiction and examples}, invariant, mutual exclusion and deadlock freedoom can be identified by looking at one single state, don't need to look at history, \videoeight{1550}{Explanation: Safety properties that need history}, \enquote{\alert{there is no bad prefix}}%, once come to state that breaks condition, then one has a finite prefix of the infinite sequence of states
																\item \script{799}{Examle: Traffic light (ff.)}
																\begin{itemize}
															...
															\begin{itemize}
																...
																\begin{itemize}
																	...
																	\begin{itemize}
																		\item \videoeight{5022}{Difference false and Empty Set}, the empty set is as an atomic proposition is not the same as false as an as a propositional formula, just as everything satisfies true, no such thing satisfies false,
																	\end{itemize}
																\end{itemize}
																\item \script{837}{$(2^{AP})^\omega$ is a safety property} and \scriptthree{12}{Other Version} and \videoeight{5186}{Explanation}, premise always false, all finite words are bad prefixes, can choose empty set as the set of bad prefixes
															\end{itemize}
														\end{itemize}
													\end{minipage}
												}
										...
										child [level distance = 8cm] {
												node {Prefix closure
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	\item \script{851}{Definition}
																	\item $E$ is a safety property \textit{iff} $cl(E) = E$
																	\begin{itemize}
																		\item \script{857}{Proof (ff.)}
																	\end{itemize}
																	\item \script{849}{Examples}
																\end{itemize}
															\end{minipage}
														}
													}
														...
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	...
																	\end{itemize}
																	\item \script{727}{Definition}, $always\;\phi$, $never\;\neg\phi$, invariantly the not bad thing holds true, every state on path of transition system which always starts in initial state satisfies the invariant condition, which means the label satisfies it
																	\begin{itemize}
																		\item \enquote{no bad state will be reached}
																	...
																	\end{itemize}
																	\item \script{738}{Satisfaction of invariants (ff.)}
																\end{itemize}
															\end{minipage}
														}
													}

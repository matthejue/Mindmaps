...
\begin{minipage}[t]{0.19\linewidth}
  ...
  \begin{betterlist} 
		...
		\item bei \cul{a U B} geht es nur darum, dass in dem Trace etwas existiert, was es erfüllt. Bei \nul{a v b}, angeben welcher Trace von welchem von beiden erfüllt ist, falls von beiden ist es unnötig, bei oder reicht einer von beiden. Formel mit einem von beiden Operanden entfernt schreiben
...
\begin{minipage}[t]{0.19\linewidth}
	...
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item P ::= Sum\textsubscript{i€I} p\textsubscript{i}.P\textsubscript{i} (summation over finite index set I) | P | Q (parallel composition) | (nu a) P (restriction (binds a name)) | !P (replication)
			\begin{betterlist}
		...
		\end{betterlist}
	\end{betterlist}
	\inlinebox{Lockset Method (complete but unsound) cont.}
  \begin{betterlist}
    \item unsound cont.
		\begin{betterlist}
...
% 		}
		...
		\adjustbox{scale=0.5}{
			% \begin{noindent}
    \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\Large}, box align=top]
(nu d1 d2)( // mu s.un!(mu a.lin+{max: lin!integer.lin!integer.lin?integer.a, 
           //                  quit: end}).s
  (nu client server)( // mu a.lin+{max: lin!integer.lin!integer.lin?integer.a, 
                   //         quit: end}
    un d2?s.s > {max: s?x.s?y if x > y then s!x.d1!s else s!y.d1!s,
                quit: 0}

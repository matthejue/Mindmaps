...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\end{betterlist}
		\includegraphics[width=0.7\linewidth]{./figures/hybrid_assymetric-symmetric_systems.png}
	\end{betterlist}
	\fbox{RSA}
	\begin{betterlist}
...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\end{betterlist}
		\item \alert{Invasive Fault Injection Attacks}
		\begin{betterlist}
			\item High cost. E.g. Removing layers of an IC, scanning by electron microscopes, small microprobes. Destructive. Modifies attacked IC itself
			\item \underline{Attacks:}
			\begin{betterlist}
				\item \alert{Signal injection:} Use probes to enforce certain logical values / flip logical values. Purpose: Modify control signals / data values on the chip. Modify memory contents. Forcefully bypass security protection
				\item \alert{Disconnect nets in the circuit by laser cutting}
				\item \alert{Modify nets by wire bonding}
				\item \alert{Destroy transistors / wires by high voltage between two probes}
			\end{betterlist}
			\item \underline{Other attacks:}
			\begin{betterlist}
				\item \alert{Get physical access to all structures of the IC:} Decapsulation. Use of acids to remove layers one by one. Image each layer before removing it
				\item \alert{Reverse Engineering:} Provide knowledge about design of IC. Layout reconstruction from images. Build netlist. Find out all functions of IC
				\item \alert{Memory Extraction:} Read contents of ROM from layout reconstruction. Scan contents of EEPROM/FLASH
				\item \alert{Micro Probing:} Microprobes can also provide side channel outputs. Listen to control lines. Listen to data bus
			\end{betterlist}
			\item \underline{IC Modifications:}
			\begin{betterlist}
				\item \alert{Decapsulation}
				\item \alert{Laser Cutting:} Not completely destructive. Selective exposure of lower layers. Selectively disconnect nets
				\item \alert{Test Point Creation:} Cut test points into IC. More spots for micro probing / signal injection below top layer. Access more signals
				\item \alert{Wire Bonding:} Use laser cutting to expose net / cut test point for bonding target. Add wire for signal insertion or for modifying circuit paths as needed
			\end{betterlist}
		\end{betterlist}
	\end{betterlist}
\end{minipage}
\begin{minipage}[t]{0.2\linewidth}
	\fbox{Countermeasures}
	...
	\begin{betterlist}
		\item \alert{Non-Invasive Fault Injection countermeasures}
		\begin{betterlist}
			\item add voltage and frequency sensors to detect voltage / frequency glitching attacks
		\end{betterlist}
		\item \alert{Semi-Invasive Fault Injection countermeasures}
		\begin{betterlist}
			\item ambient light sensors and switches against decapsulation of the device
		\end{betterlist}
		...
		\begin{betterlist}
			...
			\includegraphics[width=\linewidth]{./figures/probing_attempt_detector.png}
...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		\item trade-off between testing and security
		\begin{betterlist}
			\item \underline{testing goal:} Make internal behaviour externally observable.
			\item \underline{security goal:} Keep internal behaviour secret.
		\end{betterlist}
		...
		\item \underline{Testing requires internal signals of the circuit to be:}
		\begin{betterlist}
			\item \alert{controllable} (assign arbitrary values to the signal) and
			\item \alert{observable} (observe signal changes at the circuit outputs)
		\end{betterlist}
		...
		\begin{betterlist}
			...
			\begin{betterlist}
				\item \underline{TDO:} Test Data Output
				\item \underline{TDI:} Test Data Input
				\item \underline{TMS:} Test Mode Select
				\item \underline{TCK:} Test Clock
				\item \underline{TRST:} Resets TAP Controller
			\end{betterlist}
			\item \underline{Three Basic Modes:}
			\begin{betterlist}
				\item \alert{EXTEST mode (external test):} Testing interconnects
				\item \alert{INTEST mode (internal test):} Testing modules
				\item \alert{BYPASS mode:} Replace scan chain of the module by a single FF. \underline{Purpose:} Skip module testing and Shorten overall scan chain
			\end{betterlist}
			\item Parts of shift register may be connected with \enquote{corresponding circuit register}
			\begin{betterlist}
				\item \underline{Update operation:} Copy from shift register into circuit register
				\item \underline{Capture operation:} Copy from circuit register into shift register
				\item \underline{Usual procedure:}
				\begin{betterlist}
					\item \alert{Test mode:} shift data in and update
					\item \alert{Functional mode:} Perform normal circuit operations
					\item \alert{Test mode:} Capture and Shift data out
				\end{betterlist}

				\includegraphics[width=0.3\linewidth]{./figures/internal_module_registers.png}
				\includegraphics[width=0.7\linewidth]{./figures/connecting_jtag_devices.png}
				\item \underline{Simplest version:}
				\begin{betterlist}
					\item \alert{Daisy chaining:} Serially connected via TDI, TDO. Control signals TMS, TCK, TRST are the same
					\item Long chain, long delays
					\item More complex structures possible
				\end{betterlist}
...
\end{minipage}
\begin{minipage}[t]{0.2\linewidth}
	\fbox{Counterfeit / Trojan Detection and Avoidance Continue}
	\begin{betterlist}

				...
				node {Hardware Security Attacks and Countermeasures
						\resizebox{\textwidth}{!}{
							\begin{minipage}[t]{12cm}
								\begin{itemize}
									\item \script{252}{Side-channel and fault injection}
								\end{itemize}
							\end{minipage}
						}
					}
				...
				child {
						node {Fault Injection
								\resizebox{\textwidth}{!}{
									\begin{minipage}[t]{12cm}
										\begin{itemize}
											\item \script{252}{Definition}, using side-channel inputs
											\item \script{253}{Side channel inputs for fault injection}
										\end{itemize}
									\end{minipage}
								}
							}
				...
				child {
						node {Side Channel Attacks
								\resizebox{\textwidth}{!}{
									\begin{minipage}[t]{12cm}
										\begin{itemize}
											\item \script{252}{Definition}, observing side channel outputs
											\item \script{253}{Side channel outputs for side channel attacks}
										\end{itemize}
									\end{minipage}
								}
							}
								...
								\resizebox{\textwidth}{!}{
									\begin{minipage}[t]{12cm}
										\begin{itemize}
											...
											\item \alert{Microarchitectural states} = processor states not defined in the ISA, e.g.:
											\begin{itemize}
												\item States of Functional Units
												\item States of reservation stations
												\item Cache contents
												\item etc.
											\end{itemize}
										\end{itemize}
						...
						child {
								node {Meltdown and Spectre
										\resizebox{\textwidth}{!}{
											\begin{minipage}[t]{12cm}
												\begin{itemize}
													\item \script{199}{Main idea}, exploited vulnerabilties
													\item \script{249}{Summary}
												\end{itemize}
											\end{minipage}
										}
									}
								child {
										node {Meltdown
												\resizebox{\textwidth}{!}{
													\begin{minipage}[t]{12cm}
														\begin{itemize}
															\item based on out-of-order executions (superscalar processor)
															\item \script{200}{Overview (f.)}
															\item \script{226}{What it uses}
															\item \underline{Implementation details:}
															\begin{itemize}
																\item \script{228}{Step 1: Unauthorized read, probe\_array and cache data (f.)}
																\begin{itemize}
																	\item \script{227}{Precondition and Code}
																\end{itemize}
																\item \script{230}{Step 2: Flushing the cache lines, Measure access times}
																\item \script{232}{Exception handling (pr.)}
																\item \script{233}{Race condition}
																\begin{itemize}
																	\item \script{234}{Steps in the race}
																\end{itemize}
															\end{itemize}
															\item \script{235}{Leak whole memory}
															\item \script{236}{Summary and Visualization}
															\item \script{238}{Countermeasures}
														\end{itemize}
													\end{minipage}
												}
											}
										...
										child {
												node {Reservation stations (RSs) and Functional Units (FUs)
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	\item \script{203}{Definition}, Retiring phase
																	\begin{itemize}
																		\item \script{204}{Illustration}
																	\end{itemize}
																\end{itemize}
															\end{minipage}
														}
										...
										child {
												node {Tomasulo Scheduling
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	\item \script{205}{Tomasulo Scheduling (ff.)}
																\end{itemize}
															\end{minipage}
														}
													}
										...
										child {
												node {Virtual memory management with paging
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	\item \script{221}{Definition}
																	\item \script{222}{Adress Translation Illustration}
																	...
																	\begin{itemize}
																		\item \script{224}{Virtual address space of processes}, Advantage of including kernel space into process page tables: TLB does not need to be flushed for privileged accesses to kernel space, e.g. system calls
																		\item \script{225}{When an exception is thrown}
																	\end{itemize}
																\end{itemize}
															\end{minipage}
														}
										...
										child {
												node {Instruction and data caches
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	\item \script{226}{Definition}, \alert{cache hit} and \alert{cache miss}
																\end{itemize}
															\end{minipage}
														}
													}
								...
								child {
										node {Spectre
												\resizebox{\textwidth}{!}{
													\begin{minipage}[t]{12cm}
														\begin{itemize}
															\item based on speculative executions, \script{199}{Note}
															\item \script{239}{Overview} and \script{241}{More}
															\item \script{242}{Code (ff.)}
															\item \script{247}{Other Application Scenarios}
															\item \script{248}{Countermeasures}
														\end{itemize}
													\end{minipage}
												}
											}
						...
						\resizebox{\textwidth}{!}{
							\begin{minipage}[t]{12cm}
								\begin{itemize}
									...
									\end{itemize}
									\item \script{194}{Criteria for \enquote{Good} Ciphers}
								\end{itemize}
							\end{minipage}
						}
					}
														...
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{16cm}
																\begin{itemize}
																	...
																	\end{itemize}
																	\item \script{181}{Encryption and Decryption} and \scripttwo{1}{Notes}, \videonine{610}{$n$ in which residue class ring do computations}, \videonine{688}{Block based cipher}, block size depends, more than 1000 bits, \videonine{728}{Meaning block and number}, \videonine{843}{Never encryption by iterated multiplication}, \videonine{988}{Complexity}, 																	\begin{itemize}
																		\item \script{182}{Key Generation} and \scripttwo{2}{Notes}, \videonine{1065}{Non-trivial task}, \videonine{1152}{Key Generation: Computations no in Z mod n Z}, \videonine{1228}{How find public exponent}, \videonine{1272}{Take simple number for public exponenet}, small number of ones in the binary representation, \videonine{1342}{Why it has to be invertable}, \videonine{1377}{How can compute this}, extended euclids algorithm to compute inverse, \videonine{1841}{Why could break RSA if could factor n in p and q}, \videonine{1939}{One way function of RSA}
																		\item \script{183}{Example including Key Generation} and \scripttwo{3}{Notes}, \videonine{1574}{Choice of e}, \videonine{1674}{public key is both...}
																	\end{itemize}
																	\item \script{184}{Security} and \scripttwo{4}{Notes}, \videonine{1986}{Use Extented Euklids algorithm to compute inverse}, \videonine{2096}{Number bits of prime numbers fixed}, \videonine{2309}{Prime number table too huge}, density not decreasing that fast
																	\item \script{185}{Correctness}, Euler's Theorem and \scripttwo{5}{Notes} and \videonine{2667}{Explanation}, \videonine{2810}{Follows from Euler's Theorem}, \videonine{2865}{Alternative way to get multiplicative inverse}, under condition that one knows phi of n, \videonine{2898}{If this times a not equal to 1}, then there's no multiplicative inverse, because then gcd is not 1, \videonine{2935}{Alternative way to compute private key d}
																	\begin{itemize}
																		\item \script{186}{Proof}, \scripttwo{6}{Fake proof} and \videonine{2954}{Explanation}, \scripttwo{7}{Real Proof for RSA}
																		and \videonine{3201}{Explanation}
																		\begin{itemize}
																			\item \scripttwo{8}{Proof of Eulers Theorem (f.)} \videonine{4018}{Explanation}, \videonine{4661}{Multiplying with a permutes elements}
																		\end{itemize}
																	\end{itemize}
																	\item \script{187}{Symmetry Property} and \scripttwo{10}{Notes}, \videonine{5414}{In not simple version a hash of message is encrypted}, \videonine{5587}{Also not send as plain text}
																	\item \script{192}{Attacks (pr. ff.)}
																\end{itemize}
															\end{minipage}
														}
													\end{minipage}
										...
										child {
												node {Fast Modular Exponentiation
														\resizebox{\textwidth}{!}{
															\begin{minipage}[t]{12cm}
																\begin{itemize}
																	\item \script{188}{Definition}
																	\item \script{189}{Algorithm}
																\end{itemize}
															\end{minipage}
														}
													}

...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\item \cul{pretty printer:} \verb|gofmt hello.go| (outputs to console), \verb|gofmt -w hello.go| (writes to same file)
	\end{betterlist}
	\inlinebox{Syntax}
	\begin{betterlist}
	...
	\end{betterlist}
	\inlinebox{Terminology}
	\begin{betterlist}
		\item \nul{Concurrency versus Parallelism}
		\begin{betterlist}
			\item \cul{Parallelism:} Make programs run faster by making use of additional CPUs (parallel hardware)
			\begin{betterlist}
			...
			\end{betterlist}
			\item \cul{Concurrency:} Program organized into multiple threads of control. Threads may work independently or work on a common task
			\begin{betterlist}
		...
		\end{betterlist}
		\item \cul{Thread:} Independently sequentially executing code
		\item \nul{Thread state:}
		\begin{betterlist}
		...
		\end{betterlist}
		\item \cul{Multithreading:} Alternating execution of multiple threads on one CPU
		\item \cul{Scheduling:} Strategy to switch between running and waiting threads
		\begin{betterlist}
			\item \cul{Preemptive scheduling:} Every thread gets a certain slice of time to run, then it is preempted and a waiting thread is selected to run
			\item \cul{Cooperative scheduling:} A thread runs until a blocking command is encountered, then a waiting thread is selected to run
		\end{betterlist}
	\end{betterlist}
	\inlinebox{Multi-threading}
	\begin{betterlist}
		\item \nul{Blocking commands:}
		\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item a \cul{path of execution} is described by a sequence of individual program states. The transition between the current and the next states is indicated with \verb|-->|. E.g. \verb|Main.Running --> (Main.Running, A.Waiting)|
		\end{betterlist}
	\end{betterlist}
	\inlinebox{Lambda's (anonymous functions)}

	\adjustbox{scale=0.5}{
		% https://github.com/cmhughes/latexindent.pl/issues/132
		% \begin{noindent}
    \begin{dnumberedcodebox}[minted language=go,minted options={autogobble, fontsize=\small}]
      go func() {
        // content
      }()
      bFunc := func() {
	...
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item \nul{rules for exchanging messages:}
			\begin{betterlist}
...
\end{minipage}
\begin{minipage}[t]{0.2\linewidth}
	\inlinebox{Channels of Channels}
	\begin{betterlist}
	...
	\end{betterlist}
	\inlinebox{Non-deterministic Choice (\texttt{select})}
	\begin{betterlist}
    ...
    %\end{noindent}   }
		...
		\item \nul{Selection with prioritization:} put the case that should be priotised several times
		\item \nul{\texttt{default}:}
	...
	\end{betterlist}
	\inlinebox{Where it can all go wrong} \exercisenotes{/home/areo/Documents/Studium/Semester_2_Master/Concurrency/slides/bonus/2024_05_03_tutorial_1.md}
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\adjustbox{scale=0.5}{
				% \begin{noindent}
            \begin{dnumberedcodebox}[minted language=text,minted options={autogobble, fontsize=\small}, box align=top, title=data race visible]
                 Main        T
            1.               y!
            2.               w(x)
            3.   w(x)
          ...
          %\end{noindent}   }
			...
			\adjustbox{scale=0.5}{
				% \begin{noindent}
            \begin{dnumberedcodebox}[minted language=text,minted options={autogobble, fontsize=\small}, box align=top, title=data race not visible]
                 Main        T
            1.               y!
            2.               w(x)
            3.               y?
...
func signal(qsem *QSem) {
  ...
  if qsem.curr < qsem.q {
	...
	\begin{betterlist}
		...
		\item \nul{Differences to Mutex:}
		\begin{betterlist}
			\item \alert{Mutex:} A mutex has the concept of ownership, meaning the thread that locks a mutex must be the one to unlock it. This is crucial for preventing issues like deadlocks or priority inversions.
			\item \alert{Semaphore:} A semaphore does not have ownership rules. Any thread can increment or decrement the semaphore, making it more flexible but also easier to misuse.
		\end{betterlist}
	\end{betterlist}
	...
	\end{betterlist}
	\inlinebox{Go-style mutexes behave like semaphores}
	\begin{betterlist}
	...
	\begin{betterlist}
		...
		\item \alert{Static analysis}: Predict all possible program runs without actually executing the program, (usually) sound, \nul{but overapproximative:} if there is a bug, it will be flagged, but also non-bug may be flagged
		...
		\begin{betterlist}
			...
			% \begin{betterlist}
			%   \item \nul{in other words:} two read/write operations on some shared variable are \alert{conflicting} if the operations happen in different threads and at least one of the operations is a write
			% \end{betterlist}
		...
		\begin{enumerate}
			...
			\begin{betterlist}
				\item \cul{Program order Condition}: Let $P$ be some trace and $P'$ be a permutation of $P$. $P'$ preserves program order if, for each thread $T$, $P \downarrow T = P' \downarrow T$
				\begin{betterlist}
					\item \nul{notation}: for a trace $P$ and thread $T$, we write $P \downarrow T$ for the list of events in thread $T$ contained in $P$
				\end{betterlist}
			\end{betterlist}
			\item \alert{Lock semantics must be maintained}:
			\begin{betterlist}
				\item \cul{Lock Semantics Condition}: Let $P$ be some trace and $P'$ be some reordering of $P$. If $𝑃' = P_1 acq(y) P_2 acq(y) P_3$, then there must be some event $rel(y)$ in $P_2$. If $P' = P_1 rel(y) P_2$, then there must be some $acq(y)$ such that $P_1 = P'_1 acq(y) P''_1$ and there is no $rel(y)$ in $𝑃''_1$
			\end{betterlist}
			\item \alert{Last writer must be maintained}:
			\begin{betterlist}
				\item \cul{Last Writer Condition}: Let $P$ be some trace with a read event $r(x)$ and $P'$ be some reordering of $P$. The event $r(x)$ must have the same \alert{last writer} in $P'$ as in $P$
				\begin{betterlist}
...
\end{minipage}
\begin{minipage}[t]{0.2\linewidth}
	\inlinebox{Efficient dynamic data race prediction methods}
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\begin{betterlist}
				\item \nul{Observations:} To enforce the critical section order we add the event set $ES_{rel(y)}$ of some release event to the event set $ES_{acq}(y)$ of some subsequent acquire event, To enforce the program order, we accumulate events within one thread (in essence, building the transitive closure)
...
\begin{minipage}[t]{0.2\linewidth}
	\begin{betterlist}
		...
		\begin{betterlist}
          ...
          %\end{noindent}   }
			...
			\begin{betterlist}
				\item if two locksets are disjoint: $LS(c(x)_i) \cap LS(c(x)_j) = \{\}$ then the conflicting events $c(x)_i$ and $c(x)_j$ represent a data race
			\end{betterlist}
	...
	\end{betterlist}
	\inlinebox{Dynamic deadlock prediction}
	\begin{betterlist}
		% \item \nul{assumptions:} 
		% \begin{betterlist}
		...
		\begin{betterlist}
          ...
          %\end{noindent}   }
			...
			\item \nul{Precision:}
			\begin{betterlist}
				\item \nul{False positive possible:} Due to the \alert{write-read dependency}, events in thread $T_1$ must happen before the events in thread $T_2$
				\item \nul{False negative possible:} \alert{Cross-thread critical} section that includes several events due to the \alert{fork/join dependency}. However, the computation of lock dependencies is \alert{agnostic} to \alert{cross-thread critical sections}. \script{161}{Example}
			\end{betterlist}
...
\begin{minipage}[t]{0.2\linewidth}
	\inlinebox{Futures and Promises}
	\begin{betterlist}
...
\end{minipage}
\begin{minipage}[t]{0.2\linewidth}
	\inlinebox{Message Passing Concurrency} \lecturenotes{/home/areo/Documents/Studium/Semester_2_Master/Concurrency/slides/bonus/Message_Passing_Concurrency_(2024_06_26).md} \additionalslides{/home/areo/Documents/Studium/Semester_2_Master/Concurrency/slides/bonus/Message_Passing_Concurrency_(2024_06_26).pdf}
	\begin{betterlist}
		\item \nul{Concurrency Flavors}
		\begin{betterlist}
			\item \alert{Shared Memory Concurrency}:
			\begin{betterlist}
				\item processes interact by reading and writing shared variables
				\item locking etc. needed to demarcate critical regions
			...
			\end{betterlist}
			\item \alert{Message Passing Concurrency}:
			\begin{betterlist}
				\item processes interact by sending and receiving messages on
				\item shared communication channels
				\item \nul{expressiveness:} shared variables may be implemented using message passing
				\begin{betterlist}
					\item model a reference by a thread and channels for reading and writing
					\item reading on the \enquote{read} channel returns the current value
					\item writing on the \enquote{write} channel spawns a new thread with the new value that manages the two channels from then on
				\end{betterlist}
			\end{betterlist}
		\end{betterlist}

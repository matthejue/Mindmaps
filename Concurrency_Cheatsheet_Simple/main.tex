...
(nu client server) // mu s.lin +{max: lin!integer!integer.?integer.s,
                //           quit: end}
d1!true.client < abs.client!-4.client!2.client?x. printInteger!x.d1!true.client < quit
|  un d2?_.server > {max server?x.server?y.if x > y then server!x else server!y,
                   quit: 0}
)
    \end{dnumberedcodebox}
    %\end{noindent}
    ...
    %\end{noindent}
		...
		\item \nul{linear syntax with linear type:}
		\vspace{-0.25cm}

		\adjustbox{scale=0.5}{
			% \begin{noindent}
    \begin{dnumberedcodebox}[minted language=text, minted options={autogobble, fontsize=\large}, box align=top]
      (nu x1 x2)( // x1 = lin ?bool
        lin x1(y) | x2<true>
      )
    \end{dnumberedcodebox}
    %\end{noindent}

...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\includegraphics[width=0.4\linewidth]{./figures/arrassig.png}

		\includegraphics[width=0.225\linewidth]{./figures/havoc.png}

		\includegraphics[width=0.3\linewidth]{./figures/assu.png}
		\begin{betterlist}
	...
	\begin{betterlist}
		\item \script{230}{Ultimate Refereee with example (ff.)}
		\item \script{233}{Interactive verification, Deductive verification, Automated verification}
		\begin{betterlist}
			\item \alert{interactive verification} (input: every proof step, check mistake in proof and tells what left to prove but have to figure out oneself)
			\item \alert{deductive verification} (input: invariants, contracts: requires and ensures pairs)
			\item \alert{automated verification} (input: program and property, find invariants and contracts automatically)
		\end{betterlist}
	\end{betterlist}
	\fbox{Control-flow graphs}
	\begin{betterlist}
		\item \underline{short definition of CFG:} automaton over the alphabet of statements
		...
		\item \script{438}{Assert Statement with Illustration}
...
\begin{minipage}[t]{0.2\linewidth}
	...
	\begin{betterlist}
		...
		\item \color{orange}an \alert{abstract reachability graph} is a graph $(AC, T)$ where AC is a set of abstract configurations and $T \subseteq AC \times Stmt \times AC$, such that
		\begin{enumerate}
	...
	\begin{betterlist}
		...
		\begin{betterlist}
			\item Partition configurations into finitely many equivalence classes (\script{369}{Visualisation}). Build a graph whose nodes are the equivalence classes
		...
		\end{betterlist}
		\item \script{371}{Obvious theorems and lemmas}
		...
		\begin{betterlist}
			...
			\item To ensure termination in abstract domains that do not satisfy the ACC, we introduce widening. If the abstract domain satisfies the ACC, widening can also help to terminate in fewer iterations
...
\begin{minipage}[t]{0.2\linewidth}
	\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item the following \script{404}{rules} define an abstract post operator for the non-relational domain induced by a value abstraction $(A_{\mathbb{Z}}, \sqsubseteq_{\mathbb{Z}}, \gamma_{\mathbb{Z}}, \alpha_{\mathbb{Z}})$:
			\begin{betterlist}
		...
		\begin{betterlist}
			...
			\item function is monotone and the definition of the function is closely related to the theorem about the set of reachable configurations RC: \script{415}{Relationship to set of reachable configurations RC (f.)}
		\end{betterlist}
		...
		\begin{betterlist}
			\item In order to guarantee termination in all cases, we require that the lattice satisfies the ACC. However, \script{426}{Example (f.)} shows, the algorithm may also terminate in some cases if the lattice does not satisfy the ACC
		\end{betterlist}
		\item \color{orange}If $(Y, \sqsubseteq_Y)$ is a complete lattice, and $X$ is any set, then $(X\rightarrow Y, \sqsubseteq)$ is also a complete lattice, where $X \rightarrow Y$ is the set of all functions from $X \rightarrow Y$, and $f \sqsubseteq g$ iff for all $x \in X$, we have $f(x) \sqsubseteq_Y g(x)$. \script{429}{Proof}\color{black}
...
\begin{minipage}[t]{0.2\linewidth}
	\begin{betterlist}
		...
		\begin{betterlist}
			\item \script{524}{Canonical Transition formulas for statements from lecture}
		\end{betterlist}
		...
		\begin{betterlist}
			...
			\item carry out this idea in the fifth constraint. \script{530}{Example}
		\end{betterlist}
		...
		\begin{betterlist}
			...
			\begin{betterlist}
				...
				\end{betterlist}
			\end{betterlist}
			\item \script{536}{Extensions}: The limitation to annotations of the form $a_{\ell} \cdot  x + b_{\ell} \cdot  y + c_{\ell} \geq 0$ is very restrictive. Using a single inequality we are not even able to state an equality like, e.g., $x = 0$. A straightforward extension is to use a boolean combination of linear inequalities
			...
			\item \script{537}{Example}. \script{541}{End result}
			\begin{betterlist}
	...
	\begin{betterlist}
		\item \script{546}{Questions} and \script{547}{Answers}, 1, 2 can is not enough, is like satisfaible and valid. 1, 3 is only exit locations. 2, 4 is error and exit locations
		...
		\begin{betterlist}
			\item \color{orange}\alert{RankTerminate:} Let $P$ be a program. If every while loop of $P$ has a ranking function then $P$ is terminating. \script{553}{Proof}\color{black}
		\end{betterlist}
	...
	\begin{betterlist}
		...
		\end{betterlist}
		\item \script{563}{Example for just ranking function (ff.)}
		\item in lasso programs the loop sometimes does not have a ranking function but there is a ranking function for the combination of the loop and a given Floyd-Hoare annotation. For these programs we synthesize a ranking function together with a loop invariant. \script{571}{Constraints for ranking function and invariant}: There exist $\vec{r}, r_0, \vec{s}, s_0$ such that:
		\begin{betterlist}
...
\begin{minipage}[t]{0.2\linewidth}
    ...
    \item wie infeasability proof genau umgesetzt wird: trace zur error location in ssa bringen, dann minimal unsatisfiable core builden und alle Statements abstrahieren, die sich nicht im core befinden. Dann einfach abstract post auf der abstraction des trace ausführen, angefangen mit true (ist more elegant proof als direkt auf den Statements auszuführen)
    \item Cegar was man da macht, um B zu bekommen: nicht Formeln von Floyd Hoare annotation nehmen, sondern trace zu error location nehmen und in SSA bringen, erstmal feasiblity check, wenn negativ, dann minimal unsatisfiable core berechnen, alle Statements die nicht im core sind abstrahieren, dann strongest post auf auf abstraction des trace ausführen, angefangen mit true und sich ergebende Formlen dem Set B hinzufügen
    \item Trace abstraction was man da macht um generalisierte Floyd Hoare automaton: zu bekommen: nicht Formeln von Floyd Hoare annotation des Program automaton nehmen, sondern trace zu error location nehmen und in SSA bringen, erstmal feasiblity check, wenn negativ, dann minimal unsatisfiable core berechnen, die statements, die im core sind landem im finalen generalisierten floyd hoare automaten
